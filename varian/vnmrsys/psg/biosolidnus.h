 ...
 * 2. nimax, ni2max, ni3max - Define the maximum increments for each dimension
 *                          These determine the range of valid increments in your NUS schedule
 *                          (e.g., nimax=64 means increments 0-63 are valid for F2)
 ...
 *   nimax=64, ni2max=32, ni3max=1, nrows=500, SPARSE='y'
 * 
 * For 2D experiment with full sampling of 128 points, using 64 NUS points:
 *   nimax=128, ni2max=1, nrows=64, SPARSE='y'
 * 
...
static int nus_nimax_vals[MAXDIM-1];  // FIXED: renamed from nus_ni_vals
...
static int nus_get_index_from_ix(int ix) {
    ...
    for (int i = 0; i < nus_maxdim-1; i++) {
        if (nus_nimax_vals[i] > 1) active_dims++;  // FIXED: renamed variable
    }
...
static int nus_read_schedule(void) {
    ...
    for (int i = 0; i < nus_nrows; i++) {
        ...
        for (int j = 0; j < nus_maxdim-1; j++) {
            if (values[j] < 0 || values[j] >= nus_nimax_vals[j]) {  // FIXED: renamed variable
                printf("ERROR: Line %d dim %d value %d out of range [0,%d]\n", 
                       line_num, j+1, values[j], nus_nimax_vals[j]-1);  // FIXED: renamed variable
                fclose(fsparse_table);
                return -1;
            }
...
static void nus_print_summary(void) {
    ...
    for (int i = 0; i < nus_maxdim-1; i++) {
        if (i == 0) {
            printf("  F2/d2: nimax_s=%d, sw=%.1f Hz, const_delay=%.3f ms\n", 
                   nus_nimax_vals[i], nus_sw_vals[i], nus_d_const[i]*1000);  // FIXED: renamed variable
        } else if (i == 1) {
            printf("  F1/d3: ni2max_s=%d, sw=%.1f Hz, const_delay=%.3f ms\n", 
                   nus_nimax_vals[i], nus_sw_vals[i], nus_d_const[i]*1000);  // FIXED: renamed variable
        } else if (i == 2) {
            printf("  F3/d4: ni3max_s=%d, sw=%.1f Hz, const_delay=%.3f ms\n", 
                   nus_nimax_vals[i], nus_sw_vals[i], nus_d_const[i]*1000);  // FIXED: renamed variable
        }
    }
    ...
    if (nus_nrows > 10) {
        printf("... and %d more points, ending with these 10 \n", nus_nrows - 10);
        for (int i = nus_nrows-10 ; i < nus_nrows; i++) {
         printf("%5d |", i+1);
         for (int j = 0; j < nus_maxdim-1; j++) {
             double time_ms = (nus_d_const[j] + nus_sel[i][j]/nus_sw_vals[j]) * 1000;
             printf("   %4d |      %6.3f |", nus_sel[i][j], time_ms);
...
static int nus_init_sequence(int maxdim) {
    ...
    /* Get experimental parameters - FIXED: use nimax_s, ni2max_s, ni3max_s */
    nus_nimax_vals[0] = (int)getval("nimax_s");
    if (maxdim >= 3) nus_nimax_vals[1] = (int)getval("ni2max_s");
    if (maxdim >= 4) nus_nimax_vals[2] = (int)getval("ni3max_s");
    ...
    for (int i = 0; i < maxdim-1; i++) {
        if (nus_nimax_vals[i] > 1) {  // FIXED: renamed variable
            expected_full_sampling *= nus_nimax_vals[i];  // FIXED: renamed variable
        }
    }
    ...
    /* Validate nimax parameters */
    for (int i = 0; i < maxdim-1; i++) {
        if (nus_nimax_vals[i] <= 0) {  // FIXED: renamed variable
            printf("ERROR: ni%smax_s must be > 0 for %dD experiment\n", 
                   (i==0) ? "" : (i==1) ? "2" : "3", maxdim);
            return -1;
        }
    }
    ...
    for (int i = 0; i < maxdim-1; i++) {
        if (i == 0) {
            printf("nimax_s=%d ", nus_nimax_vals[i]);  // FIXED: renamed variable
        } else if (i == 1) {
            printf("ni2max_s=%d ", nus_nimax_vals[i]);  // FIXED: renamed variable
        } else if (i == 2) {
            printf("ni3max_s=%d ", nus_nimax_vals[i]);  // FIXED: renamed variable
        }
    }
    printf("\n");
    ...
    int total_nimax_product = nus_nimax_vals[0];  // FIXED: renamed variable
    for (int i = 1; i < maxdim-1; i++) {
        total_nimax_product *= nus_nimax_vals[i];  // FIXED: renamed variable
    }
    
    if (nus_nrows == total_nimax_product && nus_nrows < expected_full_sampling) {
        printf("NOTE: nrows equals nimax product but less than expected full sampling\n");
 ...
 * tRF: constant RF duty cycle parameter
...
//    printf("NUS VALIDATING SAFETY!!!");
    ...
    for (int i = 0; i < nus_nrows; i++) {
        ...
        /* Note this error checking works fine with dps but then nus_d_const[i][2] values are accumulating after 'go', leading to error. why? CMR 8/5/25 */
        for (int j = 0; j < nus_maxdim-1; j++) {
            double evo_time = nus_d_const[j] + nus_sel[i][j] / nus_sw_vals[j];
/* debug */
//            printf("nus_d_const %.3f nus_sel %i nus_swval %.3f ",nus_d_const[j],nus_sel[i][j],nus_sw_vals[j]);
//            printf("j=%i evo_time=%.3f ms\n",j,evo_time);
...
/* debug
        printf("row i=%i total_time=%.3f ms\n",i,total_time);
*/            
