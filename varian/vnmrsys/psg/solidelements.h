...
//====================================================================
   ...
   if (strlen(chXtra) == 0) {
      sprintf(name0,"%s%s%s%s",varType,chUP,chSeqType,chDescr);
      return name0;
   }
   sprintf(name0,"%s%s%s%s%s",varType,chXtra,chSeqType,chLOW,chDescr);
   return name0;
}
...
//================================================================
   ...
   if (chnl > 0) {
      sprintf(name1,"%s%s%s%s%s",varType,chID,chLOW,chSeqType,chDescr);
   }
   else {
      sprintf(name1,"%s%s%s%s",varType,chUP,chSeqType,chDescr);
   }
   return name1;
}
...
//
// check that the 'array' parameter of a pulse sequence is a set of
//   descending phase strings (e.g. "phase2,phase1,phase" or
//   "phase3,phase2" etc)
//
...
void check_array() {
   ...
   char phases_string[sizeof(one) + sizeof(two) + sizeof(three) + 3];

   phases_string[0] = 0;

   strcat(phases_string, three);

   if (strlen(phases_string) > 0 && strlen(two) > 0)
       strcat(phases_string, ",");

   strcat(phases_string, two);

   if (strlen(phases_string) > 0 && strlen(one) > 0)
       strcat(phases_string, ",");

   strcat(phases_string, one);
       
   char array_parameter[256];

   array_parameter[0] = 0;
    
   getstr("array", array_parameter);
  
// CMR 9/13/24 this error checking does not permit arraying parameters at all, even if ni=0 and phase=0
// The desired behavior is to allow normal array construction (as in 1D experiments) if user is 
// collecting the "first row" of the 2D/3D/4D experiment, which is true 
// when phase=phase2=phase=ni=ni2=ni3=0  
// So I am commenting this out now 9/13/24 12:38 pm so that we can continue with testing
// Recompiled hXX as an example and it now works as intended, but this is brute force
// Barry will need to work with Chad to figure out the appropriate logical arguments later
//   if (strcmp(array_parameter, phases_string) != 0) {
//        
//      abort_message("'array' parameter is expected to be this (%s) but is this (%s)!", phases_string, array_parameter);
//   }
}
