...
CP make_cp(CP cp)
{
   ...
   if (!strcmp(cp.ch,"fr") ) {
      ...
      mean=cp.a2;
       ...
       printf("make_cp() Error: unknown direction! (%s)\n", cp.ch);
       psg_abort(1);
   }
...
void _initcp_(CP cp, double lamplitude)
{
   ...
   if (strcmp(cp.ch,"fr") == 0)
      ...
      chRamp = cp.to;
       ...
       printf("_initcp_() Error: unknown direction! (%s)\n", cp.ch);
       psg_abort(1);
   }
...
void _setcp_(CP cp)
{
   ...
   if (strcmp(cp.ch,"fr") == 0) {
      ...
      chConst = cp.fr;
       ...
       printf("_setcp_() Error: unknown direction! (%s)\n", cp.ch);
       psg_abort(1);
   }
   if (strcmp(chRamp,"obs") == 0)       nchRamp = 1;
   else if (strcmp(chRamp,"dec") == 0)  nchRamp = 2;
...
void MPinitializer(MPSEQ *mp,int npw,int nph,int no,int na,int ng,int nBase,int nSuper)
{
   ...
   if (PWRF_DELAY > 0.0) mp->n90 = 40;  // BDZ and CMR: changed from 16 to 40 on 11-20-23
   else mp->n90 = 40;                   // BDZ and CMR: changed from 8 to 40 on 11-20-23
   mp->preset1 = 0;
   mp->preset2 = 0;
...
// If dph = 0.0, write the pulse as a block.
      ...
      if (dph == 0.0 ) {
         phase = roundphase(ph+seq.phBase[i%seq.nph] + seq.phSuper[iph],360.0/1024); // CMR 7/21/2017
         if (printmode == 1)
   ...
   if (strcmp(r.sh,"c") == 0 && fabs(r.of)  < 1.0) {
      ...
      while (nrpuls > 0) {
         if (nrpuls == r.n) {
            ...
            phase = roundphase(ph,360.0/1024); // CMR 7/21/2017
            fprintf(fp,"%10.1f %10.3f %6.1f\n",90.0,phase,aCurrent);
            t += (r.n90 - 1)*DTCK;
            nrpuls -= (r.n90 - 1);
            r.phInt += (r.n90 - 1)*dph;
...
//Calculate the shape and count ticks down from the initial "nrpul"
   ...
   while (nrpuls > 0) {
      if (nrpuls == s.pars.n) {               // Always write the first step
         ...
         phase = roundphase(ph+phShapeCurrent,360.0/1024);  // CMR changed from 8192 to 1024 7/21/2017
...
// Supply INOVA Start Delays
   ...
   if (PWRF_DELAY > 0.0) s.pars.n90 = 40;  // BDZ and CMR: changed from 16 to 40 on 11-20-23
   else s.pars.n90 = 40;                   // BDZ and CMR: changed from 8 to 40 on 11-20-23
   s.pars.preset1 = 0;
   s.pars.preset2 = 0;
