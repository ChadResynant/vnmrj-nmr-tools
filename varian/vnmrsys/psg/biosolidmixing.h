/*
 * biosolidmixing.h - Header file for homonuclear mixing sequences
...
static MixingType get_mixing_type(const char* mix_str) {
    ...
    if (!strcmp(mix_str, "r2t")) return MIX_R2T;
    if (!strcmp(mix_str, "dream")) return MIX_DREAM;
...
static void setup_mixing_sequences(MixingParams *params, const char* nucleus) {
    ...
    switch(params->type) {
        ...
        case MIX_R2T:
        case MIX_DREAM:
            params->r2t = getr2tckl("Xr2t", 0, 0.0, 0.0, 0, 1);
            strcpy(params->r2t.ch, "obs");
            params->dec_mix = getdseq("Hmix");
            break;
...
static double calculate_mixing_duty_cycle(MixingParams *params, double base_duty, double base_time) {
    ...
    switch(params->type) {
        ...
        case MIX_R2T:
        case MIX_DREAM:
            total_duty += 2*getval("tZF") + getval("tXr2t_in") + 
                         getval("tXr2t_mix") + getval("tXr2t_out") + 2.0*getval("pwX90");
            break;
...
// R2T/DREAM mixing implementation
static void execute_r2t_dream_mixing(MixingParams *params) {
    decpwrf(params->aH90);
    obspwrf(params->aX90);
    decon();
    rgpulse(getval("pwX90"), params->phMix1, 0.0, 0.0);
    ...
    if (params->type == MIX_DREAM) {
        decpwrf(params->aH90);
        decon();
        obspwrf(params->aX90);
        rgpulse(getval("pwX90"), getval("ph90Xdream1"), 0.0, 0.0);
        decoff();
    }
    
    _dseqon(params->dec_mix);
    _r2tckl(params->r2t, getval("phXr2t"), getval("phXr2t"), getval("phXr2t"));
    _dseqoff(params->dec_mix);
    
    if (params->type == MIX_DREAM) {
        decpwrf(params->aH90);
        decon();
        obspwrf(params->aX90);
        rgpulse(getval("pwX90"), getval("ph90Xdream2"), 0.0, 0.0);
        decoff();
    }
...
static void execute_mixing_sequence(MixingParams *params) {
    ...
    switch(params->type) {
        ...
        case MIX_R2T:
        case MIX_DREAM:
            execute_r2t_dream_mixing(params);
            break;
