...
// CMR 4/7/23 fixing soft pulse version of this phase cycle
// saving as a separate pulse sequence called hYXXsoft.c to avoid unintended consequences with hard pulse version
// will harmonize at a later date (ha ha)
// symptom was that the indirect C dimension with soft pulse option on was giving half frequency artifacts for 
// the signals slightly outside of the soft pulse bandwidth, corresponding to those seeing the soft pi pulse as a pi/2
// so the solution is to do spin pinging 00 01 10 11 on the soft/hard pair for the 4 step phase cycle
// then add spin temp on N for steps 5 to 8 so that the 8 step can still be used effectively for calibrations
...
static int table6[8]  = {0,0,0,0,2,2,2,2};                          // phYyx
static int table7[8]  = {0,0,0,0,0,0,0,0};                          // phXyx

static int table8[8]  = {3,3,1,1,3,3,1,1};                          // phXmix1
static int table9[8]  = {0,0,1,1,2,2,3,3};                          // phXmix2
static int table21[8] = {0,0,0,0,0,0,0,0};                          // phXspc5
static int table22[8] = {0,0,1,1,0,0,1,1};                          // phXspc5ref
...
static int table15[8] = {0,1,0,1,0,1,0,1};                          // phXsoft 
static int table16[8] = {0,0,1,1,0,0,1,1};                          // phXhard 
static int table17[8] = {0,0,0,0,0,0,0,0};                          // phYhard 
static int table18[8] = {1,1,1,1,1,1,1,1};                          // phXmix1_soft
static int table19[8] = {0,0,0,0,1,1,1,1};                          // phXmix2_soft
static int table20[8] = {2,0,0,2,1,3,3,1};                          // phRec_soft 
...
void pulsesequence() {
    ...
    if ((phase1 == 1) || (phase1 == 2)) {
        ...
        //hypercomplex... CMR 4/7/23 not obvious to me why we would want to shift soft/hard pair here
