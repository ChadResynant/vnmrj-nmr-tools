...
static int table15[8] = {0,2,1,3,2,0,3,1};   // phRechard
...
if (!strcmp(T1,"y"))
 ...
 // External trigger on channel 1 number 1
   sp1on(); sp4on(); delay(2.0e-6); sp1off(); delay(2.0e-6);  // CMR 10/11/23 use sp1on() for scope trigger and F19 Tomco amp
   rcvroff();
