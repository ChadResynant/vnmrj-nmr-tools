/*  hXXX.c - A sequence to provide XXX homonuclear correlation with the option of using RAD, SPC5, POSTC7 or RFDR mixing
    ...
    CMR 7/9/25: added several duty cycle protection checks for better robustness
*/
...
static int table1[8] = {0,2,0,2,0,2,0,2};                           // phH90
...
static int table10[16]={1,1,2,2,3,3,0,0,3,3,0,0,1,1,2,2};	        // phXmix3
static int table11[8] ={0,1,2,3,2,3,0,1};                           // phXmix4
static int table6[16] ={1,0,3,2,3,2,1,0,3,2,1,0,1,0,3,2};           // phRec
...
void pulsesequence() {
   ...
   if (qXspcn % NXspcn != 0) {
    ...
    // Dutycycle Protection  CMR 7/9/25 added d3 and correct error logic below for RAD aka DARR mixing power deposition
    tRF = getval("tRF");

   if (!strcmp(mMix, "rad")) {
       duty = 4.0e-6 + getval("pwH90") + getval("tHX") + d2 + d3 + tRF + (getval("aHmix")/4095) * (getval("aHmix")/4095) * (tXmix + tXmix1) +
              2.0*getval("pwX90") + getval("ad") + getval("rd") + at;
       duty = duty/(duty + d1 + tXmix + tXmix1 + 4.0e-6);
   }
   else if (!strcmp(mMix, "c7")) {
       duty = 4.0e-6 + getval("pwH90") + getval("tHX") + 2.0*getval("tZF") + c7.t + d3 + tRF +
              d2+ tXmix1 + c7ref.t + getval("ad") + getval("rd") + at;
       duty = duty/(duty + d1 + 4.0e-6 + 2.0*getval("tZF"));
   }
   else if (!strcmp(mMix, "spc5")) {
       duty = 4.0e-6 + getval("pwH90") + getval("tHX") + d2 + d3 + tRF +
              2.0*getval("pwX90") + tXmix1 + getval("ad") + getval("rd") + at;
       duty = duty/(duty + d1 + 4.0e-6);
   }
    if (duty > 0.05) {
        abort_message("Duty cycle %.1f%% >5%%. Check d1, d2, d3, tRF, at, tXmix, tXmix1. Abort!\n", duty*100);
    }
    else {
...
/* NUS Pt.2 - Read-in NUS schedule file: '~/vnmrsys/nus_sched/hXXX_S.hdr_3' */
    {
 	...
 	char check_str[MAXSTR];
   	...
   	int nrows = getval("nrows");   /* used for NUS in 4Ds; added here just for bookkeeping  CMR 7/9/25 */ 
	...
	getstr("SPARSE",sparse);
        if(sparse[0]=='y') {
            ...
            printf(" row#   ni  d2(ms) ni2  d3(ms)  \n");
            printf("-----------------------------------------\n");
            for(i = 0; i < nrows; i++) {
                printf("%5d %4d %6.3f %4d %6.3f\n", i+1, sel[i][0], (d2const+sel[i][0]/sw1)*1000., sel[i][1], (d3const+sel[i][1]/sw2)*1000.);
            }
            printf("-----------------------------------------\n");
            printf(" row#   ni  d2(ms) ni2  d3(ms) \n");
            printf("-----------------------------------------\n");
