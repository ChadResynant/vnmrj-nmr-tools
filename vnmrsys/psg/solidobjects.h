...
#ifdef MATCH_MASTER
   double obsstep = 360.0/(8192*PSD);  // a bugfix?
#else
...
#ifdef MATCH_MASTER
   tsadd(mp.tstep,phint,8192*PSD);  // bugfix?
#else
...
#ifdef MATCH_MASTER
   double obsstep = 360.0/(8192*PSD); // bugfix?
#else
   double obsstep = 360.0/8192;
#endif
...
#ifdef MATCH_MASTER
   tsadd(mp.tstep,phint,8192*PSD);  // a bugfix?
#else
   tsadd(mp.tstep,phint,8192);
#endif
...
// ph1Xhssmall
   ...
   ph1 = ph1*(PSD*8192)/360.0;  // a bugfix?
#else
   ph1 = roundphase(ph1,360.0/8192); 
   ph1 = ph1*8192/360.0;
#endif
...
// ph2Xhssmall
   ...
   ph2=ph2*(PSD*8192)/360.0;   // bugfix??

#else

   ph2 = roundphase(ph2,360.0/8192);
   ph2 = ph2*8192/360.0;
...
// Set mp.va and initialize 2048 steps (90 degrees)
   ...
   initval(2048.0*PSD, mp.va);  // a bugfix?
...
// BDZ: 2-26-24: updated this signature to match 3.1A. Our stuff was
//   ridiculously out of date and any stock pulse sequence that called
//   this routine previously would have crashed due to the wrong number
//   of stack parameters passed.
...
// Set DUMBO phase array   
   ...
   double obsstep = 360.0/(PSD*8192);  // a bugfix??
...
// First half of the array
   ...
   for (i = 0; i < steps/2; i++) {
      dumbo[i] = 0.0;  // BDZ: this is a bugfix from OVJ master
...
// BDZ 10-13-23
//   Code inspection shows some serious code rot. I have a bug fix
//   into Dan Iverson but he hasn't accepted it yet.
//   There is only one stock OVJ pulse sequence that calls this and that
//   code looks like it was never completed.
...
// DSEQ module. 
   ...
   if (!strcmp(d.seq,"waltz")) {
      sprintf(ch2,"%s",d.w.ch);    // BDZ fixed this: used to use d.s.ch!
   }
   if (!strcmp(d.seq,"paris")) {   // BDZ added this clause
      sprintf(ch2,"%s",d.p.ch);
   }

   // BDZ: the above clause looks out of date. There are many more sequences
   // than those listed. In solids it is only missing spinal2. In biosolids
   // it is missing a ton. Not sure which of those use this code.
